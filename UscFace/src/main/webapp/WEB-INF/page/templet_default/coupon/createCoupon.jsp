		...
		<form>
			<div class="createCoupon">
				<div>
					<select class="form-control dropdown-control"
						onchange="changeCouponChild()" id="sel-couponList">
				...
				<div class="coupon" style="text-align: center; padding: 10px 0;">
					<c:forEach items="${cTemp}" var="item" begin="0" end="0">
						<img id="coupon-img" src="${item.img}" alt="优惠券" title="优惠券" />
					</c:forEach>
				</div>
			</div>
			<div class="inputsControls">
				...
				<div class="textInput">
					<input class="form-control" id="input-createCoupon-num" type="tel"
						inputmode="numberic" pattren="[0-9]*" placeholder="请输入发放卡券数量" />
				</div>
			</div>
			<div class="inputsControls">
				<div class="textHead">
				...
				<div class="dateInput">
					<input placeholder="日/月/年" class="form-control start-date"
						type="text" id="start-date" readonly /> <label>&nbsp;至&nbsp;</label>
					<input placeholder="日/月/年" class="form-control end-date"
						type="text" id="end-date" readonly />
				</div>
			</div>
			<div class="inputsControls">
				...
				<div class="textInput">
					<textarea id="textarea1" rows="5" cols="20"
						class="textarea-control area-useExplain" placeholder="."></textarea>
				</div>
			</div>
			...
			<div class="button-controls">
				<input class="btn-default btn submit" type="button"
					onclick="submitCoupon()" value="添加优惠" /> <input
...
// 				alert("couponCreate.do?tempetId="+couponId+"&totalNum="+num+"&startTime="+timeStart+"&endTime="+timeEnd+"&useRule="+useExplain);
			//	window.location.href = "showInfo.do?info=您的优惠券提交成功!<br><br>等待我们的客服人员审核制作！谢谢！";
				window.location.href = "couponCreate.do?tempetId="+couponId+"&totalNum="+num+"&startTime="+timeStart+"&endTime="+timeEnd+"&useRule="+useExplain;
